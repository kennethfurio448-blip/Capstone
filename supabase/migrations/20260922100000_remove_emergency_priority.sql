alter table public.emergency_requests
    drop constraint if exists emergency_requests_priority_check;
...
begin
    ...
    select
        ...
        record.quantity, trim(record.unit),
        nullif(record.expiration_date, '')::date,
    ...
    where not exists (
        select 1
        from jsonb_array_elements(p_data -> 'medtrackMedicalSupplies') entry
    ...
    where not exists (
        select 1
        from jsonb_array_elements(p_data -> 'medtrackMedicalEquipment') entry
        where entry ->> 'id' = item.id
    );
    ...
    where not exists (
        select 1
        from jsonb_array_elements(p_data -> 'medtrackMobilityAssets') entry
        where entry ->> 'id' = item.id
    );
    ...
    select
        ...
        nullif(trim(record.destination), ''),
        nullif(trim(record.remarks), ''),
    ...
    where not exists (
        select 1
        from jsonb_array_elements(p_data -> 'medtrackBorrowTransactions') entry
        where entry ->> 'id' = item.id
    );
    ...
    select
        ...
        trim(record.request_type), trim(record.location),
    ...
    from jsonb_to_recordset(p_data -> 'medtrackEmergencyRequests') as record(
        ...
        location text, contact_person text, contact_number text,
    ...
    where not exists (
        select 1
        from jsonb_array_elements(p_data -> 'medtrackEmergencyRequests') entry
        where entry ->> 'id' = item.id
    );
end;
$$;
