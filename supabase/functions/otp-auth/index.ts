...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (action === "verify-registration") {
      ...
      validateEmail(email);
      if (!fullName || username.length < 4 || password.length < 8 || !["admin", "staff"].includes(role)) throw new Error("The account information is invalid.");
    ...
    if (action === "reset-password") {
      ...
      const password = String(body.password || "");
      if (!challenge.target_user_id || password.length < 8) throw new Error("Use a password containing at least 8 characters.");
    ...
    console.error(error);
    const message = error instanceof Error ? error.message : "Unable to complete verification.";
    const status = message.includes("authentication") || message.includes("administrator") ? 401 : 400;
